...
if __name__=="__main__":
    ...
    ingest = input()
    outgest = input()
    ingest = list(map(int, ingest.split()))
    outgest = list(map(int,outgest.split()))
    Total = 0
    for i in range(T):
        Total += ingest[i] - outgest[i]
        outgest[i] = Total # now total at instance is stored at outgest list
    print(max(outgest)) # return max of list
